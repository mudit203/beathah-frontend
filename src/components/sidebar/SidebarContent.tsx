...
const SidebarContent: React.FC = () => {
  ...
  const updatedSidebar = (sidebar as SidebarItem[])
    .map((route) => {
      if (route.routes) {
  ...
  return (
    <div className="py-4 text-gray-500 dark:text-gray-400">
      <Link href="/dashboard" className="text-gray-900 dark:text-gray-200">
        {mode === "dark" ? (
          ...
          route.routes ? (
            ...
            <li className="relative" key={route.name}>
              <Link
                ...
                target={('outside' in route && route.outside) ? "_blank" : "_self"}
                className={`px-6 py-4 inline-flex items-center w-full text-sm font-semibold transition-colors duration-150 hover:text-emerald-700 dark:hover:text-gray-200 ${
                  pathname === route.path ? "text-emerald-500 dark:text-gray-100" : ""
                }`}
                rel="noreferrer"
              >
                {pathname === route.path && (
